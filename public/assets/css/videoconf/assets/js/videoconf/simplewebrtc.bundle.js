...
function SimpleWebRTC(opts) {
	...
	connection.on('lemosbeta', function(data) {
        $(".videoContainer").removeClass("profesor");
        if(webrtc.connection.getSessionid() == data){
            $("#my-videocontainer").addClass("profesor");
        }
        setTimeout(function(){
            $("#"+data).addClass("profesor");
        },5000);
	});

    connection.on('wordChanged', function(data) {
        var divToMove, profesor;
        if(jQuery("#profesor #"+data.peer).length == 0){
            divToMove = jQuery("#"+data.peer).wrap('<p/>').parent().html();
            jQuery("#"+data.peer).unwrap();
            profesor = jQuery("#profesor").html();
            jQuery("#profesor").empty();
            jQuery("#"+data.peer).remove();
            jQuery("#remotes").append(profesor);
            jQuery("#profesor").append(divToMove);
            jQuery("#"+data.peer+" video")[0].muted = 0;
            jQuery("#remotes video").each(function(index,value){
               value.muted = 1;
            });
            jQuery("#profesor video")[0].muted = 0;
            document.getElementById('localVideo').muted = 1;
        }
    });

    connection.on('clients', function(data) {
        console.log("CLIENTS");
        jQuery.each(data,function(k,v){
            jQuery("#name-"+k).html("@"+v.user);
        })
    });
...
(function() {
  ...
  // popular enh to be bundled in a third party lib, but not be part of
...
module.exports = function (constraints, cb) {
    ...
    if (window.navigator.userAgent.match('Chrome')) {
        ...
        if (isCef || (chromever >= 26 && chromever <= maxver)) {
            ...
            chrome.runtime.sendMessage("babcpegmckgleclkcfhmoaemfbmcnfjp" ,{ type: 'getScreen',id:pending },{},function(message){
