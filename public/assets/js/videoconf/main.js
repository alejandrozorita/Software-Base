...
function start_main() {
    ...
    function call_fix_prof (peer_id) {
        console.log(peer_id);
        var remotes = document.getElementById('remotes');
        var profesor = document.getElementById('profesor');

        if (!jQuery(profesor).find('div#'+peer_id).length) {
            console.log("Rectificacion del Profesor");
            //jQuery(remotes).find('div#' + peer_id).hide();
            jQuery(profesor).html(jQuery('div#' + peer_id).clone());
            //jQuery(profesor).find('div#' + peer_id).show();
            jQuery(remotes).find('div#' + peer_id).remove();
        }
    }

    function aj_fix_prof(peer_id) {
        $.ajax({
            url: serverObject.has_word,
            type: 'POST',
            data: { room : room, session: peer_id},
            xhrFields: {
                withCredentials: true
            },
            success: function (peers) {
                console.log('Rectificando Profesor');
                console.log(peers);
                if (peers.success) {
                    call_fix_prof(peer_id);
                }
            }
        });
    }
                ...
                webrtc.on('videoAdded', function (video, peer) {
                    ...
                    $.ajax({
                        ...
                        success: function (peers) {
                            ...
                            setTimeout(aj_fix_prof(peer.id), 200);
    ...
    function initMyCam(room){
        ...
        $.ajax({
            ...
            success: function (peers) {
                ...
                setTimeout(aj_fix_prof(peerID), 200);
                webrtc.startLocalVideo();
            },
            error: function (err) {
                console.log(err);
